...
- **Control-binding governance gate** (ADR-0061, RFC-0004): `scripts/governance/check_control_bindings.py` (pure, offline, 94% covered), `.github/control-triggers.yml`, `docs/governance/applicability-matrix.yml`, a `Governance Checks` CI step, and `make check-control-bindings`. Blocks a PR that fires a control trigger without declaring the matching control; enforces the 2-skill budget + 3-domain atomicity smell; conditional controls (SOX) exempt when out of scope (Issue #85, ADR-0061)
