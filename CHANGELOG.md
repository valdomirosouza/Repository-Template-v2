...
- `.github/workflows/release.yml` — `build-release-image` now runs only when `CONTAINER_REGISTRY` is configured; otherwise a new `image-publish-skipped` job emits an explanatory notice instead of failing, so registry-less releases stay green (Issue #96, RFC-0009)
- `.github/workflows/release.yml` — publish release images to **GHCR via the built-in `GITHUB_TOKEN`** (no stored registry secrets); lowercase the image name (GHCR requirement) and add `cosign --yes` for non-interactive keyless signing. `CONTAINER_REGISTRY` variable set to `ghcr.io` (Issue #96, RFC-0010)
...
- `docs/repository-maintenance/asdd-15-phase-simulation-record.md` — validation record: a reversible, sandboxed dry-run of the full 15-phase Agentic Spec-Driven Delivery lifecycle (ADR-0058) with per-phase evidence; all phases 0–14 passed, human gates honored, Phase 10 correctly N/A (ADR-0058)
...
- `.github/workflows/release.yml` + `release-please-config.json` + `.release-please-manifest.json` — migrate release-please to v4 manifest mode, restoring the custom changelog sections that the removed inline `changelog-types` input had silently dropped (Issue #93, RFC-0006)
