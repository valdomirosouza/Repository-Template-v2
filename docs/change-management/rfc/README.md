...
| [RFC-0013](RFC-0013-branch-protection-main.md)                       | Branch protection for `main`                              | ⚠️ Under Review — **implementation diverged** (live ruleset has 14 checks vs 9; see RFC-0014 + audit) | 2026-06-07 |
| [RFC-0014](RFC-0014-release-please-ruleset-bypass.md)                | release-please ruleset + bypass                           | Accepted (⚠️ diverges from live `main-branch-protection` ruleset; reconciliation pending)             | 2026-06-07 |
...
## Open reconciliation items (from 2026-06-16 audit)

- **RFC-0013 / RFC-0014 vs live ruleset** — the enforced `.github/rulesets/main.json` is named
  `main-branch-protection` with **14 required checks** and **no `Detect Secrets`** and **no bypass
  actor**, contradicting RFC-0013/0014 (9 checks, `main-protection`, `Detect Secrets` required).
  A follow-up RFC should document the move to the 14-check enforced set, or the ruleset should be
  reconciled to the RFCs. The versioned `org-protection-ruleset.json` still encodes the old
  9-check design — two conflicting sources of truth.
- **RFC-0014 §4 internal contradiction** — §2a states "no bypass actor" while §4 ships a
  `bypass_actors` block; the live ruleset has none. Remove the contradictory §4 block.
