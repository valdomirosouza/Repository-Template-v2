...
> **Version:** 2.14.0 | **Status:** Active | **License:** MIT
...
3. **Drive the whole lifecycle:** `/deliver [dry-run|code] [language] specs/<domain>/<your-spec>.md` drives the spec through all 15 phases via the `phase-executor` subagent, and emits `reports/<slug>/FINAL-REPORT.md` with requirement-traceability, per-phase timing, gate verdicts, and the open-HITL list. Mode defaults to `dry-run`:
   ...
   - **`language`** (optional, default `PYTHON`) — the stack to build in: `PYTHON` (`src/`), `JAVA`/`GO` (`services/<name>/`), `NODE`/`TYPESCRIPT` (`frontend/<app>/`), `IAC` (Terraform/Ansible → `infrastructure/`), or another stack. It selects the code location + validation targets (e.g. `/deliver code java specs/…`); all gates and guardrails are language-agnostic.
