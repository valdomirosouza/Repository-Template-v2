...
> **Version:** 2.4.0 | **Last updated:** 2026-06-06
...
| ADRs          | `docs/adr/`                               | ADR-0001–ADR-0058 all binding; 0030–0041 cover RTK, Agentic SDLC onboarding, sub-agent registry, session durability, escalation protocol, AI-assisted CI review, cyber defense, governance gate, learn stage, tool registry, maturity model, context graph; 0042 K8s probes; 0043–0046 OTel agentic observability; 0047–0051 Secure-by-Design (spec contracts, zero-trust tooling, behavioral monitoring, adversarial testing, model behavioral contracts); 0052 Agentic SDLC E2E workflow; 0053–0057 runtime correctness, governance contracts, HOTL ops, release hardening, repo hygiene; 0058 Agentic Spec-Driven Delivery (15-phase) |
...
Pre-0f: For Phase 1–2 (Conception/Discovery) tasks — read docs/process/WORKFLOW.md §Phase 1–2
        before creating discovery.md or nfr.md artefacts. Use Spec-as-PR governance (not the
        runtime HITL gateway) for pre-code artefacts. See docs/process/HITL-GOVERNANCE.md.
```

If a spec cannot be found after two searches → emit `[HITL-ESCALATE]` (§14).

**Full 15-phase (0–14) lifecycle reference:** `docs/process/WORKFLOW.md` (ADR-0052, ADR-0058). **Canonical model + positioning:** `docs/sdlc/agentic-spec-driven-delivery.md`
...
- **ALWAYS** run `make test-abuse-cases` (or `uv run pytest tests/abuse_cases/ -m abuse_case`) before any PR touching `src/agents/` or `src/guardrails/`. Never reduce the abuse case test count (ADR-0050).
...
| OTel agent spans, GenAI conventions, HITL trace | `skills/observability/otel-instrumentation.md`    | Any agent/harness instrumentation, span hierarchy, or llm.inference work (OTEL-001, ADR-0043–0046) |
